...
const flatMap$5 = dual(2, (self, f) =>
  isNone(self) ? none$4() : f(self.value)
);
const flatten$2 = flatMap$5(identity);
...
const TypeId$7 = Symbol.for("effect/Context");
const ContextProto = {
  [TypeId$7]: { _S: (_) => _ },
  [symbol](that) {
    if (isContext(that)) {
...
const _empty$5 = makeContext(new Map());
...
const TypeId$6 = Symbol.for("effect/Chunk");
...
const _empty$4 = makeChunk({ _tag: "IEmpty" });
...
const get$5 = dual(2, (self, index) =>
  index < 0 || index >= self.length ? none$4() : some(unsafeGet(self, index))
);
...
const make$i = (...entries) => fromIterable$3(entries);
...
const parallelCollectionMake = (dataSource, blockedRequest) =>
  new ParallelImpl(make$e([dataSource, Array.of(blockedRequest)]));
...
const failures$1 = (self) =>
  reverse$1(
    reduce(self, empty$c(), (list, cause) =>
...
const causeEquals = (left, right) => {
  ...
  while (isNonEmpty(leftStack) && isNonEmpty(rightStack)) {
    const [leftParallel, leftSequential] = pipe(
      ...
      reduce([empty$9(), empty$c()], ([parallel, sequential], cause) => {
...
const flattenCause = (cause) => flattenCauseLoop(of$1(cause), empty$c());
const flattenCauseLoop = (causes, flattened) => {
  while (1) {
    const [parallel, sequential] = pipe(
      causes,
      reduce$6([empty$9(), empty$c()], ([parallel, sequential], cause) => {
        const [par, seq] = evaluateCause(cause);
        return [
          pipe(parallel, union$1(par)),
          pipe(sequential, appendAll$1(seq)),
...
const make$a = (options) => ({ [TracerTypeId]: TracerTypeId, ...options });
const tracerTag = Tag(Symbol.for("effect/Tracer"));
const spanTag = Tag(Symbol.for("effect/ParentSpan"));
const ids = globalValue("effect/Tracer/SpanId.ids", () => make$f(0));
class NativeSpan {
  name;
  parent;
  context;
...
const currentContext = globalValue(
  ...
  () => fiberRefUnsafeMakeContext(empty$d())
);
...
const TypeId$2 = Symbol.for("effect/MutableHashMap");
...
const fromHashMap = (backingMap) => {
  ...
  map.backingMap = make$f(backingMap);
  return map;
};
const empty$2 = () => fromHashMap(empty$8());
const get$1 = dual(2, (self, key) => get$2(self.backingMap.current, key));
const has = dual(2, (self, key) => isSome(get$1(self, key)));
const set$1 = dual(3, (self, key, value) => {
...
const TypeId$1 = Symbol.for("effect/Console");
const consoleTag = Tag(TypeId$1);
...
const get = dual(2, (self, fiberRef) => {
...
class MetricRegistryImpl {
  ...
  getCounter(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has(key))) {
        pipe(this.map, set$1(key, counter));
      }
      value = counter;
    }
    return value;
  ...
  getFrequency(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has(key))) {
        pipe(this.map, set$1(key, frequency$1));
      }
      value = frequency$1;
    }
    return value;
  ...
  getGauge(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has(key))) {
        pipe(this.map, set$1(key, gauge$1));
      }
      value = gauge$1;
    }
    return value;
  ...
  getHistogram(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has(key))) {
        pipe(this.map, set$1(key, histogram));
      }
      value = histogram;
    }
    return value;
  ...
  getSummary(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has(key))) {
        pipe(this.map, set$1(key, summary$1));
      }
      value = summary$1;
    }
    return value;
...
const tagged = dual(3, (self, key, value) =>
  taggedWithLabels(self, make$g(make$5(key, value)))
);
...
class FiberRuntime {
  ...
  constructor(fiberId, fiberRefs0, runtimeFlags0) {
    ...
    this._tracer = get$6(this.getFiberRef(currentServices), tracerTag);
  }
  id() {
...
const getConsole = (refs) => {
  ...
  const cnsl = get$6(defaultServicesValue, consoleTag);
  return cnsl.unsafe;
};
...
const tracerLogger = globalValue(Symbol.for("effect/Logger/tracerLogger"), () =>
  makeLogger(
    ({
      ...
      const span = flatMap$5(get(context, currentContext), getOption(spanTag));
      const clockService = map$7(get(context, currentServices), (_) =>
        get$6(_, clockTag)
      );
      if (
        span._tag === "None" ||
...
const currentLoggers = globalValue(
  ...
  () => fiberRefUnsafeMakeHashSet(make$g(defaultLogger, tracerLogger))
);
...
const input = [2, 3, 5];
const program = head$1(input).pipe(
  fromOption(() => "empty array"),
  flatMap$4((b) => divide(10, b)),
  match$2({ onLeft: (e) => `Error: ${e}`, onRight: (a) => `Result: ${a}` })
);
console.log(program);
